...
def determine_interface_type(mac: str, ip: str) -> str:
    ...
    # Use a simple heuristic: if we can't determine, just use Unknown
    # but in practice the vendor info from nmap is more reliable
    return "Unknown"
...
def reconcile_assets(hosts: List[Dict], logger=None) -> List[Dict]:
    ...
    if logger and multi_interface_count > 0:
        logger.info("Entity Resolution: consolidated %d multi-interface devices (%d total assets)",
                   multi_interface_count, len(unified))
