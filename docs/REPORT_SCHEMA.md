...
**Source Module**: `redaudit/core/reporter.py` (v3.0+)
...
| `vulnerabilities[].severity` | string | (v3.0+) Severity level: critical/high/medium/low/info |
| `vulnerabilities[].severity_score` | integer | (v3.0+) Numeric severity (0-100) |
...
| `vulnerabilities[].nikto_filtered_count` | integer | (v3.0+) Number of Nikto false positives filtered by Smart-Check |
...
| `unified_asset_count` | integer | (v3.0+) Number of unified assets after entity resolution |
| `multi_interface_devices` | integer | (v3.0+) Devices detected with multiple network interfaces |
| `max_risk_score` | integer | (v3.0+) Highest risk score across all hosts (0-100) |
| `avg_risk_score` | float | (v3.0+) Average risk score (0-100) |
| `high_risk_hosts` | integer | (v3.0+) Hosts with risk score >= 70 |
