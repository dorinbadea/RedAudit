...
- In Nuclei progress detail, `split depth X/Y` is retry split depth (fatigue cap), not target count.
- In the progress bar, the detail text reports sub-batch elapsed while the right-side elapsed timer tracks total task elapsed.
...
- Resumes are stored in the same scan folder and are updated in place. If a resume run ends as partial again, the existing `nuclei_resume.json`/`nuclei_pending.txt` is refreshed (not duplicated). In the interactive menu (**Resume Nuclei (pending)**), choose **Manage resume entries** to delete one or all stale entries directly.
- For post-run quality gates, validate artifact completeness and SIEM exports with `python scripts/check_scan_artifacts.py --run-dir <scan_folder> --strict`.
