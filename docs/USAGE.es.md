...
- Cuando se define un presupuesto de tiempo, es un **limite total de tiempo real para la fase de Nuclei** (no por lote). RedAudit ejecuta lotes de forma secuencial, limita el lote actual al tiempo restante y guarda `nuclei_resume.json` + `nuclei_pending.txt` al agotarse el presupuesto. Si no respondes, **la auditoria continua tras Nuclei** y la reanudacion queda disponible.
