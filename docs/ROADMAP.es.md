...
| Prioridad | Característica | Estado | Descripción |
| :--- | :--- | :--- | :--- |
| **Alta** | **Única Fuente de Versión** | 🎯 Planificado | Leer versión de `pyproject.toml` via `importlib.metadata` en vez de `VERSION = "x.y.z"` manual. Previene drift de versiones. Victoria rápida (~30 min). |
| **Media** | **Refactorizar `auditor.py`** | 🎯 Planificado | Dividir el `auditor.py` de 2600 líneas en módulos más pequeños: `wizard.py` (UI interactivo), `orchestrator.py` (coordinación de scan). Reduce patrón "God Object". |
| **Media** | **Aumentar Cobertura de Tests** | 🎯 Planificado | Subir cobertura de tests de ~25% a >50%. Foco en edge cases de parsers de herramientas externas (nikto, testssl, nmap). |
| **Baja** | **Distribución PyPI** | 🚧 Aplazado | Publicar en PyPI para `pip install redaudit`. Base lista (`pyproject.toml`), bloqueado por Única Fuente de Versión. |
| **Baja** | **Contenedorización** | 🚧 Aplazado | Dockerfile + Docker Compose. Aplazado hasta v4.0 o demanda explícita de usuarios. |
...
| **Baja** | **Indicador de Actividad en Net Discovery** | ✅ **Implementado (v3.5.2)** | Añade feedback visible de actividad durante las fases de Net Discovery y reduce el ruido de logs mientras la UI de progreso está activa; el ETA ahora considera timeouts (cota superior). |
...
| **Baja** | **Reinicio Obligatorio Post-Actualización** | ✅ **Implementado (v3.5.2)** | Tras instalar una actualización, RedAudit muestra un aviso grande, espera confirmación y sale para asegurar que la siguiente ejecución cargue la nueva versión limpiamente. |
